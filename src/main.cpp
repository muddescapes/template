...
static void mqtt_cb_message(esp_mqtt_event_handle_t event) {
    ...
    if (strncmp(event->data, "test", event->data_len) == 0) {
        Serial.printf("test message received");
        esp_mqtt_client_publish(event->client, MQTT_TOPIC, "test message received", 0, 0, 0);
    }
    ...
    if (strncmp(event->data, "led-status {on, off} status=on", event->data_len) == 0) {
        Serial.printf("Received override led-status on");
        esp_mqtt_client_publish(event->client, MQTT_TOPIC, "Received override led-status on", 0, 0, 0);
        turn_on_led = true;
    }

    if (strncmp(event->data, "led-status {on, off} status=off", event->data_len) == 0) {
        Serial.printf("Received override led-status off");
...
void loop() {
    ...
    if (millis() - msg_time > 500 && client) { //change the value here to change how often it publishes
        ...
        esp_mqtt_client_publish(client, MQTT_TOPIC, message1.c_str(), 0, 0, 0);
        esp_mqtt_client_publish(client, MQTT_TOPIC, message2.c_str(), 0, 0, 0);
    }
